...
def setCompRange(data,**kwargs):
    ...
    x1min, x2min, x3min : lower boundary x- value for each direction
    x1max, x2max, x3max : upper boundary x- value for each direction
    n1min, n2min, n3min : lower boundary index value for each direction (default is 0) 
    n1max, n2max, n3max : upper boundary index value for each direction (default is data.n[1,2,3])
    ...
    #get new x3 boundaries
    if 'x3min' in kwargs:
        x3min=kwargs.get('x3min')
        for k in xrange(data.n3):
            if data.x3[k] >= x3min:
                x3min= data.x3[k]
                n3min= k
                break
    else:
        n3min=kwargs.get('n3min',0)
        x3min=data.x3[n3min]

    if 'x3max' in kwargs:
        x3max=kwargs.get('x3max')
        for k in xrange(data.n3-1,1,-1):
            if data.x3[k] <= x3max:
                x3max= data.x3[k]
                n3max= k+1
                break
    else:
        n3max=kwargs.get('n3max',data.n3)
        x3max=data.x3[n3max-1]
    ...
    dictNew['nshp'] = (n3max-n3min,n2max-n2min,n1max-n1min)
    ...
    dictNew['x1r'] = dict['x1r'][n1min:n1max+1]
    ...
    dictNew['x2r'] = dict['x2r'][n2min:n2max+1]

    #set new x3 direction
    dictNew['n3_tot'] = n3max-n3min
    dictNew['n3'] = n3max-n3min
    dictNew['krange']=[x for x in range(0,dictNew['n3'])]
    dictNew['x3'] = dict['x3'][n3min:n3max]
    dictNew['dx3'] = dict['dx3'][n3min:n3max]
    dictNew['x3r'] = dict['x3r'][n3min:n3max+1]

    for var in dict['vars']:
        dictNew[var]= dict[var][n1min:n1max,n2min:n2max,n3min:n3max]
